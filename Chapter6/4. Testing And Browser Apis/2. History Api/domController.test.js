...
const { updateItemList, handleAddItem, handleItemName, checkFormValues, handleUndo, handlePopstate } = require("./domController");
...
describe("tests with history", () => {
	...
	describe("handlePopstate", () => {
		test("updating the item list with the current state", () => {
			history.pushState({ inventory: { cheesecake: 5, "carrot cake": 2 } }, "title");
			...
			expect(getByText(itemList, "carrot cake - Quantity: 2")).toBeInTheDocument();
		});
	});
});
