export class InventoryManagement {
	...
	static enterItemName(itemName) {
		return cy.get('input[placeholder="Item name"]').clear().type(itemName);
	}

	static enterQuantity(quantity) {
		return cy.get('input[placeholder="Quantity"]').clear().type(quantity);
	}
	...
	static undo() {
		return cy.get("button").contains("Undo").click();
	}

	static findAction(inventoryState) {
		return cy.get("p:not(:nth-of-type(1))").then(p => {
			return Array.from(p).filter(p => {
				return p.innerText.includes("The inventory has been updated - " + JSON.stringify(inventoryState));
			});
		});
	}
}
