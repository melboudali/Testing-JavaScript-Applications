...
describe("removing items from a cart", () => {
	test("removing existing items", async () => {
		...
		const response = await request(app).del("/carts/test_user/items/cheesecake").expect(200).expect("Content-Type", /json/);
	...
	test("removing non-existing items", async () => {
		...
		carts.set("test_user", []);
		const response = await request(app).del("/carts/test_user/items/cheesecake").expect(400).expect("Content-Type", /json/);
