const fs = require("fs");
const { generateItemRow, generateTotalRow, createInventoryValuesReport } = require("./inventoryReport");

describe("generateItemRow", () => {
	it("generate an item's row", () => {
		expect(generateItemRow({ name: "macaroon", quantity: 12, price: 3 })).toBe("macaroon,12,3,36");
		expect(generateItemRow({ name: "cheesecake", quantity: 6, price: 12 })).toBe("cheesecake,6,12,72");
		expect(generateItemRow({ name: "apple pie", quantity: 5, price: 15 })).toBe("apple pie,5,15,75");
	});

	it("commit soldout items", () => {
...
describe("createInventoryValuesReport", () => {
	it("create reports", () => {
		...
		expect(fs.readFileSync(`${__dirname}/inventoryValues.csv`, "utf8")).toBe(
			"apple pie,3,15,45\ncheesecake,2,12,24\nmacaroon,20,3,60\nTotal,,,129"
		);

		createInventoryValuesReport(items.slice(1));
		expect(fs.readFileSync(`${__dirname}/inventoryValues.csv`, "utf8")).toBe("cheesecake,2,12,24\nmacaroon,20,3,60\nTotal,,,84");

		createInventoryValuesReport(items.slice(2));
		expect(fs.readFileSync(`${__dirname}/inventoryValues.csv`, "utf8")).toBe("macaroon,20,3,60\nTotal,,,60");
	});
});
